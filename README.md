...
Before we do the implementations we need to go to to firebase 
 ...
 runtime and using these settings on the json file and it will create a FirebaseAuth instance which will be stored and retrived in the static field called Instance in the FirebaseAuth class.
 and from this instance we will be able to  call all the appropriate methods we need to authenticate such as SignInWithEmailAndPasswordAsync
...
Thats it we are done with the android implementation  usually this works but if you find yourself in a null reference exception because the FirebaseAuth static class doesnt contain any instance ☹ ,
we will need to do this the second way in this approach instead of hoping that the FirebaseAuth will hopefully find the json file and create an instance of FirebaseAuth.
We actually write the code to create the instance.this is archieved the following way so first we will need to navigate to the MainActivity.cs file


1. We create a static public Field of type FirebaseAuth call it Auth  and a private field of type FirebaseApp call it _fireApp
the code will look like this Create two static variables.
```
 private FirebaseApp _fireApp;
...
Next we move onto the iOS application ,like we did on the android app depending on where you you want to place yoour implementation
code 
